...
\uplevel{
Använd \citet{Brookshear2012csa,Brookshear2009csa} och Internet för
...
\emph{ifconfig}, för UNIX, och \emph{ipconfig}, för Windows, vara användbara.
...
\uplevel{
För följande uppgift kan kommandot \emph{ipconfig} användas för Windows och
verktyget \emph{hostname} för UNIX.
}
\question
...
\uplevel{
För följande uppgifter kan kommandot \emph{nslookup} användas för både UNIX
och Windows.
Kommandot körs i terminalen.
}
\question
...
\uplevel{
För följande uppgift kan du använda whois-tjänsten hos DomainTools\footnote{%
	URL: \url{http://whois.domaintools.com}.
} om du använder Windows eller kommandot \emph{whois} i terminalen om du
använder UNIX.
}
\question
...
\begin{parts}
	\part \Mbps\ (megabit per sekund)
	\part \MBps\ (megabyte per sekund)
\end{parts}
